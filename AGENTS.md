...
## Deployment Workflow
This repository is the live `finmodpro` project. The source of truth is GitHub, and normal delivery is `local change -> push -> pipeline deploy`; avoid manual server edits unless the task explicitly requires them. This machine IS the production server. Deployments run directly via `./scripts/deploy.sh` — no SSH needed. Never SSH to the public IP (47.85.103.76); always deploy locally or via 127.0.0.1.
