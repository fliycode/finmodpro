...
FinModPro 是一个面向金融风控场景的全栈演示项目，聚焦 **知识库管理、RAG 检索问答、风险事件抽取、权限控制与运维配置** 等核心能力。仓库采用 **monorepo** 结构，前端与后端分离开发，并接入 GitHub Actions 持续集成与部署流程。
...
- Shell deployment scripts (`scripts/deploy.sh`, `scripts/smoke-check.sh`)
...
仓库当前包含 GitHub Actions 工作流：`.github/workflows/ci-cd.yml`

默认流程：

1. 后端依赖安装与 `python manage.py check`
2. 后端测试套件执行
3. 前端依赖安装与构建
4. `main` 分支推送后触发服务器部署
...
- GitHub Actions 通过 SSH 登录目标服务器
- 在目标目录执行 `scripts/deploy.sh`
- 可结合 `scripts/smoke-check.sh` 做部署后验证

README 不包含真实服务器地址、SSH 凭据或 Secret 名值映射以外的敏感信息。
