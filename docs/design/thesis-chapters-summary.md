...
文档解析方面，PDF 主解析由 pymupdf4llm 承担，后端 fallback 使用 pypdf，DOCX 使用 python-docx 解析。智能算法方面，系统采用 RAG 技术，由文档解析、文本切块、embedding、Milvus 检索、rerank 和大模型回答组成。检索抽象层使用 LlamaIndex，通过 LlamaIndex-over-Milvus 统一向量存储接口，并支持混合检索（MySQL 全文 + 关键词回退 + RRF 融合）。系统还使用 LangGraph 编排问答流程，使用 OpenAI 兼容 provider 直连不同模型服务。
