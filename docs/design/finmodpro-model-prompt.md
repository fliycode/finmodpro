...
3. 基于 LlamaIndex-over-Milvus 的 RAG 检索
4. 基于 LangGraph 的问答编排
5. 风险事件抽取、审核、报告生成
6. 模型配置、网关观测、成本统计、评测与微调管理
7. 系统监控、告警管理、健康检查与审计日志
...
7. 模型调用使用 OpenAI 兼容 provider 直连，没有 LiteLLM 中间网关。
8. 检索使用 LlamaIndex-over-Milvus 统一架构，没有 LightRAG 或 Neo4j。
9. 文档解析链路要区分主路径与回退路径：PDF 主解析使用 pymupdf4llm，后排回退使用 pypdf。
...
- 一个把 MySQL、Redis、Milvus 等基础服务与 LangGraph、LlamaIndex 等 AI 组件组合起来的应用栈
