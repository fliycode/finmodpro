...
FinModPro 是一个面向金融风控、金融知识管理与模型运维的全栈系统，采用 Vue 3 + Vite 前端与 Django 5 + DRF 后端的前后端分离架构。系统核心围绕 **认证与 RBAC、知识库文档入库、RAG/图增强检索、金融问答、风险事件抽取、模型中台与运维观测** 展开，并通过 Docker Compose 连接 MySQL、Redis、Milvus、LiteLLM、LightRAG、MinIO、Etcd 等服务形成完整运行链路。
...
| 模型网关 | LiteLLM |
| 图增强检索 | LightRAG |
| 图存储 | 当前生产 compose 为 `NetworkXStorage`；历史设计文档中出现过 Neo4j 方案 |
...
需要特别注意：**当前生产 compose 没有 Neo4j 服务**。如果历史文档提到 “LightRAG + Neo4j”，应视为历史设计或演进方向，而不是当前已启用事实。
...
按当前 `docker-compose.prod.yml`，LightRAG 的图存储仍是 `NetworkXStorage`，向量存储为 Milvus；不要默认系统已经切换到 Neo4j 后端。
...
- LightRAG：图增强检索（当前 compose 仍使用 NetworkX graph storage）
...
- 前端是双壳、双角色产品
- 后端是多 app 分域系统
- MySQL / Redis / Milvus / LiteLLM / LightRAG / Unstructured 共同构成运行底座
