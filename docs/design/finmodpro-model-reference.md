...
| 文档解析 | 轻量解析服务内的 PyMuPDF PDF 主解析 + Unstructured-compatible element 输出 + 后端 `pypdf` fallback |
...
- **文档解析要容错**：PDF 主路径由轻量解析服务内的 PyMuPDF 承担，服务失败时后端回退到 `pypdf`
...
```text
上传文档
  -> 解析（轻量解析服务内的 PyMuPDF / 后端 pypdf fallback）
  -> 结构化文本与 metadata
  -> chunk / section chunk
  -> Milvus 向量入库
  -> RAG 检索
...
- LiteLLM：统一模型入口
- Unstructured API：轻量解析服务边界与 elements 输出契约
- PyMuPDF：PDF 主解析引擎
